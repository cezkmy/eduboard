...
@auth
<!-- Load Tailwind for the widget since Central uses Bootstrap -->
...
<style>
    /* Default variables in case Central App does not define them */
    :root {
        --accent: #10b981;
        --accent-dark: #059669;
...
>
    ...
    >
        ...
        <div class="px-5 pt-5 pb-4 flex items-center justify-between border-b border-gray-100 dark:border-white/10 shrink-0 relative">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-gray-800 dark:text-white text-xs font-black uppercase tracking-wider"
                       x-text="view === 'conversations' ? 'Central Support' : (view === 'new_conversation' ? 'New Message' : (activeConv ? activeConv.subject : 'Support'))"></p>
            ...
            <div class="flex items-center gap-1.5">
                <button type="button" x-show="view !== 'conversations'" @click="backToConversations()"
                    ...
                    class="cursor-pointer w-7 h-7 rounded-xl flex items-center justify-center text-gray-500 hover:text-gray-800 hover:bg-gray-100 dark:text-white/60 dark:hover:text-white dark:hover:bg-white/10 transition-all text-[10px] font-black">
                ...
                <button type="button" @click="open = false"
                    style="border: none; background: transparent; outline: none;"
                    class="cursor-pointer w-7 h-7 rounded-xl flex items-center justify-center text-gray-500 hover:text-gray-800 hover:bg-gray-100 dark:text-white/60 dark:hover:text-white dark:hover:bg-white/10 transition-all">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
        ...
        <div x-show="view === 'conversations'" class="flex-1 flex flex-col overflow-hidden" x-cloak>
            <div class="flex-1 overflow-y-auto p-3 space-y-1">
                <template x-if="conversations.length === 0">
                    <div class="flex flex-col items-center justify-center h-full text-center pb-8">
                        ...
                        <p class="text-gray-400 dark:text-white/30 text-xs font-bold">No messages yet</p>
                    </div>
                </template>
                    ...
                    <button type="button" @click="openConversation(conv)"
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-gray-500 dark:text-white/40 text-[10px] font-semibold truncate mt-0.5" 
                               x-text="IS_ADMIN ? conv.from_name + ' ('+conv.from_role+')' : (conv.status === 'open' ? 'Ongoing' : 'Closed')"></p>
                        </div>
            ...
            <div class="p-3 border-t border-gray-100 dark:border-white/10 shrink-0" x-show="!IS_ADMIN">
                <button type="button" @click="view = 'new_conversation'"
                    style="border: none; outline: none; background: linear-gradient(135deg, var(--accent), var(--accent-dark));"
        ...
        <div x-show="view === 'new_conversation'" class="flex-1 overflow-y-auto p-4 space-y-4" x-cloak>
            <div>
                <label class="block text-gray-500 dark:text-white/60 text-[10px] font-black uppercase tracking-wider mb-2">What is your concern?</label>
                <select x-model="newSubject" class="w-full bg-gray-50 dark:bg-white/10 text-gray-800 dark:text-white border-transparent focus:border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent)]/30 rounded-xl px-3 py-2.5 text-xs font-bold appearance-none transition-all">
                    ...
                    <option value="Billing / Refunds" class="text-gray-900">Billing / Refunds</option>
        ...
        <div x-show="view === 'chat'" class="flex-1 flex flex-col overflow-hidden" x-cloak>
            ...
            <div class="p-3 border-t border-gray-100 dark:border-white/10 shrink-0 flex gap-2">
                ...
                <button type="button" @click="sendMessage()" :disabled="!newMessage.trim() || submitting"
                    style="border: none; outline: none; background: var(--accent);"
    ...
    <button type="button" @click="toggleChat()"
        style="border: none; outline: none; background: linear-gradient(135deg, var(--accent), var(--accent-dark)); box-shadow: 0 8px 32px rgba(var(--accent-rgb),0.45);"
...
function supportChat() {
    ...
    return {
        ...
        loadConversations() {
            fetch('/support/inbox', { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                .then(r => r.json())
                .then(data => { 
        ...
        loadMessages(id, silent = false) {
            fetch(`/support/messages?ticket_id=${id}`, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                .then(r => r.json())
                .then(data => {
