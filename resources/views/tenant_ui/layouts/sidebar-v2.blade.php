        ...
        @if($user->role === 'admin')
            <div class="space-y-2">
                ...
                @if($user->hasPermission('page_admin_settings'))
                    @if($isBasic)
                    <a href="{{ route('tenant.admin.settings') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all {{ request()->routeIs('tenant.admin.settings') ? 'text-white shadow-lg' : 'hover:bg-gray-800 hover:text-white' }}" style="{{ request()->routeIs('tenant.admin.settings') ? 'background: var(--accent); box-shadow: 0 16px 28px rgba(var(--accent-rgb), 0.22);' : '' }}">
