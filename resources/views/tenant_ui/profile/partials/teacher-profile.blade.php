    ...
    <div class="flex-1 min-w-0 space-y-6">
        <div class="profile-card p-6 bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col items-center text-center">
            <div class="relative group">
                <div class="w-24 h-24 rounded-2xl bg-teal-500 text-white flex items-center justify-center font-bold text-3xl shadow-lg shadow-teal-500/20 mb-4 overflow-hidden" id="profile-photo-preview">
                ...
                {{-- Camera Icon --}}
                <label for="profile_photo_input" class="absolute bottom-2 -right-2 w-10 h-10 bg-white dark:bg-gray-700 rounded-xl border border-gray-100 dark:border-gray-600 flex items-center justify-center text-teal-600 hover:text-teal-700 cursor-pointer shadow-lg transition-all group-hover:scale-110 z-10" title="Change Profile Photo">
            ...
            <div id="photo-actions" class="hidden flex gap-2 mt-4">
                <button type="button" id="save-photo-btn" class="px-3 py-1.5 bg-teal-500 text-white text-[10px] font-bold rounded-lg hover:bg-teal-600 transition-all shadow-sm">SAVE PHOTO</button>
            ...
            <span class="px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider bg-teal-50 text-teal-600">Teacher</span>

            <div class="w-full mt-6 pt-6 border-t border-gray-50 dark:border-gray-700 text-left space-y-4">
                <div>
                    <div class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Employee ID</div>
                    <div class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ auth()->user()->employee_id ?? 'Not Set' }}</div>
                </div>
                <div>
                    <div class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Department</div>
                    <div class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ auth()->user()->department ?? 'Not Set' }}</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="profile-card p-6 bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm">
            ...
            <form method="post" action="{{ route('tenant.profile.update') }}" class="space-y-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <input name="name" type="text" value="{{ old('name', $user->name) }}" class="w-full bg-gray-50 dark:bg-gray-700/50 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-teal-500/20 transition-all" required>
                </div>
                
                <div class="space-y-1.5">
                    <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Email Address</label>
                    <input name="email" type="email" value="{{ old('email', $user->email) }}" class="w-full bg-gray-50 dark:bg-gray-700/50 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-teal-500/20 transition-all" required>
                </div>

                <div class="space-y-1.5">
                    <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Department</label>
                    <input name="department" type="text" value="{{ old('department', $user->department) }}" class="w-full bg-gray-50 dark:bg-gray-700/50 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-teal-500/20 transition-all">
                </div>

                <div class="pt-2">
                    <button type="submit" class="w-full py-3 bg-teal-500 text-white rounded-xl text-xs font-bold hover:bg-teal-600 transition-all shadow-md shadow-teal-500/20">SAVE CHANGES</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="profile-card p-6 bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm">
            ...
            <form method="post" action="{{ route('tenant.password.update') }}" class="space-y-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <input name="current_password" type="password" class="w-full bg-gray-50 dark:bg-gray-700/50 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-teal-500/20 transition-all">
                </div>

                <div class="space-y-1.5">
                    <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">New Password</label>
                    <input name="password" type="password" class="w-full bg-gray-50 dark:bg-gray-700/50 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-teal-500/20 transition-all">
                </div>

                <div class="space-y-1.5">
                    <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Confirm Password</label>
                    <input name="password_confirmation" type="password" class="w-full bg-gray-50 dark:bg-gray-700/50 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-teal-500/20 transition-all">
                </div>
